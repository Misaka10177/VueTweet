<template>
  <div id="join">
    <div class="logo">
      <div class="icon">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <g>
            <path :d="logo_x"></path>
          </g>
        </svg>
      </div>
    </div>
    <div>
      <div class="form">
        <div class="title">
          <h1>正发生</h1>
          <h2>现在就加入。</h2>
        </div>
        <div class="register">
          <!-- <div class="quick_register"> -->
          <button class="register_option" v-for="(item, index) in register_list" :key="index">
            <div class="icon">
              <svg :viewBox="item.viewBox ? item.viewBox : '0 0 24 24'" aria-hidden="true">
                <g v-html="item.icon"></g>
              </svg>
            </div>
            <div>{{ item.title }}</div>
          </button>
          <!-- </div> -->
          <div class="or">
            <div></div>
            <span>或</span>
            <div></div>
          </div>
          <button class="register_account button">创建账号</button>
          <div class="tip" v-html="tip" :class="$style.theme_color"></div>
        </div>
        <div class="login">
          <div class="title">已有账号?</div>
          <router-link to="/join/login"><button>登入</button></router-link>
          <button>
            <div class="icon">
              <svg :viewBox="logo_grok.viewBox" aria-hidden="true">
                <g>
                  <path :d="logo_grok.icon"></path>
                </g>
              </svg>
            </div>
            <div>获取Grok</div>
          </button>
        </div>
      </div>
    </div>
  </div>
</template>
<script setup lang="ts">
import { ref } from 'vue'

const tip = ref(
  '注册即表示同意<span>服务条款</span>及<span>隐私政策</span>，其中包括<span> Cookie 使用条款</span>。',
)

const logo_x = ref(
  'M21.742 21.75l-7.563-11.179 7.056-8.321h-2.456l-5.691 6.714-4.54-6.714H2.359l7.29 10.776L2.25 21.75h2.456l6.035-7.118 4.818 7.118h6.191-.008zM7.739 3.818L18.81 20.182h-2.447L5.29 3.818h2.447z',
)

const logo_grok = ref({
  icon: 'M12.745 20.54l10.97-8.19c.539-.4 1.307-.244 1.564.38 1.349 3.288.746 7.241-1.938 9.955-2.683 2.714-6.417 3.31-9.83 1.954l-3.728 1.745c5.347 3.697 11.84 2.782 15.898-1.324 3.219-3.255 4.216-7.692 3.284-11.693l.008.009c-1.351-5.878.332-8.227 3.782-13.031L33 0l-4.54 4.59v-.014L12.743 20.544m-2.263 1.987c-3.837-3.707-3.175-9.446.1-12.755 2.42-2.449 6.388-3.448 9.852-1.979l3.72-1.737c-.67-.49-1.53-1.017-2.515-1.387-4.455-1.854-9.789-.931-13.41 2.728-3.483 3.523-4.579 8.94-2.697 13.561 1.405 3.454-.899 5.898-3.22 8.364C1.49 30.2.666 31.074 0 32l10.478-9.466',
  viewBox: '0 0 33 32',
})
const register_list = ref([
  {
    name: 'google',
    icon: '<path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"></path><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"></path><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"></path><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"></path><path fill="none" d="M0 0h48v48H0z"></path>',
    title: '使用 Google 账号注册',
    viewBox: '0 0 48 48',
    // to: '/login/google',
  },
  {
    name: 'apple',
    icon: '<path d="M16.365 1.43c0 1.14-.493 2.27-1.177 3.08-.744.9-1.99 1.57-2.987 1.57-.12 0-.23-.02-.3-.03-.01-.06-.04-.22-.04-.39 0-1.15.572-2.27 1.206-2.98.804-.94 2.142-1.64 3.248-1.68.03.13.05.28.05.43zm4.565 15.71c-.03.07-.463 1.58-1.518 3.12-.945 1.34-1.94 2.71-3.43 2.71-1.517 0-1.9-.88-3.63-.88-1.698 0-2.302.91-3.67.91-1.377 0-2.332-1.26-3.428-2.8-1.287-1.82-2.323-4.63-2.323-7.28 0-4.28 2.797-6.55 5.552-6.55 1.448 0 2.675.95 3.6.95.865 0 2.222-1.01 3.902-1.01.613 0 2.886.06 4.374 2.19-.13.09-2.383 1.37-2.383 4.19 0 3.26 2.854 4.42 2.955 4.45z"></path>',
    title: '使用 Apple 注册',
    // to: '/login/apple',
  },
])
</script>
<style scoped>
#join {
  width: 100vw;
  height: 100vh;
  display: flex;
  /* justify-content: center;
  align-items: center; */

  --boundary-style: 1px solid #dadce0;
  --theme-color: rgb(29, 155, 240);
}
#join > div {
  flex: 1;
}
.title {
  font-weight: 700;
}
h1 {
  font-size: 64px;
  margin-bottom: 0.7em;
}
h2 {
  font-size: 32px;
  margin-bottom: 0.7em;
}
.logo {
  /* width: 100%;
  height: 100%; */
  display: flex;
  justify-content: center;
  align-items: center;
}
.logo .icon {
  width: 50%;
  height: 50%;
}
.logo .icon svg {
  width: 100%;
  height: 100%;
}
svg {
  width: 20px;
  height: 20px;
}
.form {
  width: 300px;
  height: 100%;
  display: flex;
  flex-direction: column;
  justify-content: center;
}
.form > div {
  width: 100%;
}
.register {
  margin-bottom: 40px;
}
.form > div > div,
button {
  width: 100%;
  height: 42px;
}
button {
  display: flex;
  justify-content: center;
  align-items: center;
  border: var(--boundary-style);
  border-radius: 20px;
  font-weight: 500;
  color: #3c4043;
  margin-top: 16px;
  transition: background 0.2s ease-in-out;
}
button:hover {
  background: rgba(15, 20, 25, 0.1);
}

button .icon {
  margin-right: 7px;
}
.or {
  display: flex;
  align-items: center;
}
.or span {
  margin: 0 6px;
}
.or div {
  flex: 1;
  height: 1px;
  background: rgba(0, 0, 0, 0.05);
}
.register_account {
  background: #000;
  color: #fff;
  font-weight: 600;
  margin-top: 0;
}
.register_account:hover {
  background: rgba(0, 0, 0, 0.8);
}
.tip {
  font-size: 12px;
  margin-top: 10px;
  color: #3c4043;
}
</style>
<style module>
.theme_color span {
  font-size: 12px;
  color: var(--theme-color);
}
</style>
